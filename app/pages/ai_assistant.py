...
class AIAssistant(param.Parameterized):
    ...
    def __init__(self, **params):
        ...
        debug_mode = os.environ.get("DEBUG", "").lower() in ("true", "1", "yes", "y")
        if debug_mode:
    ...
    def view(self):
        ...
        reset_button = pn.widgets.Button(
            name="Reset Form", button_type="danger", width=150
        )
        reset_button.on_click(
            lambda event: self._reset(event, reset_references, results_container)
        )
        ...
        execute_button.on_click(lambda event: self._execute_sql(event))
        ...
        def load_query(event):
            if (
                ...
                and 0 <= saved_queries_list.value < len(self.saved_queries)
            ):
        ...
        schema_accordion = pn.Accordion(
            ("Database Schema", pn.pane.Markdown(self.db_schema)), toggle=True
        )
        ...
        for category, examples in QUERY_EXAMPLES.items():
            ...
            for i, example in enumerate(examples):
                ...
                example_btn.on_click(lambda event, ex=example: self._use_example(ex))
...
"""
            ...
            self.status_message = (
                "<span style='color: blue;'>Generating SQL query... please wait.</span>"
            )
    ...
    def _clean_generated_sql(self, sql):
        ...
        sql = re.sub(
            r"\blab\b(?!_)", "lab_results", sql, flags=re.IGNORECASE
        )  # Don't match lab_results
    ...
    def _execute_sql(self, event=None):
        ...
        try:
            ...
            sql_to_execute = validation_result.get("fixed_query", self.generated_sql)
            if "fixed_query" in validation_result:
                self.generated_sql = sql_to_execute
                self.status_message = "<span style='color: blue;'>SQL was automatically fixed and will be executed.</span>"
        ...
        try:
            ...
            logger.info(f"Query executed successfully. Returned {rows_count} rows.")

            if rows_count == 0:
                self.status_message = "<span style='color: orange; font-weight: bold;'>NOTICE</span><br>Query executed successfully but returned <strong>zero rows</strong>. Your query might be too restrictive or there's no matching data."
            else:
    ...
    def _update_results_display(self):
        ...
        try:
            ...
            self.results_container.append(
                pn.pane.Markdown(f"Error displaying results: {error_msg}")
            )
    ...
    def _reset(self, event=None, reset_references=None, results_container=None):
        ...
        self.generated_sql = ""
        self.status_message = (
            "<span style='color: blue;'>Form cleared. Ready for a new query.</span>"
        )
        self.query_name = ""
        self.result_data = pd.DataFrame()
    ...
    def _validate_button_click(self, event=None, validation_badge=None):
        ...
        if validation_result["valid"]:
            if (
                ...
                and "couldn't be fully validated" in validation_result["error"]
            ):
                # Query is likely valid but has complex date functions
                self.status_message = (
                    "SQL Validation Status: <span style='color: orange; font-weight: bold;'>REQUIRES REVIEW</span><br><br>"
                    + validation_result["error"]
                )
                if validation_badge:
            ...
            try:
                ...
                if fixed_query:
                    ...
                    try:
                        ...
                        db_query.query_dataframe(f"EXPLAIN QUERY PLAN {fixed_query}")
                        return {
                            "valid": True,
                        ...
                        logger.error(f"Fixed query still has errors: {str(fix_error)}")
                ...
                if "no such table" in error_msg.lower():
                    ...
                    if table_match:
                        ...
                        if (
                            not bad_table.lower().endswith("s")
                            and bad_table.lower() + "s" in self.table_details
                        ):
                            plural = bad_table + "s"
                            logger.info(
                            ...
                            return {
                                "valid": False,
                                "error": f"Table '{bad_table}' doesn't exist. Did you mean '{plural}'? Our database uses plural table names (patients, vitals, etc.).",
                            }
                        ...
                        if bad_table.lower() == "lab" or bad_table.lower() == "labs":
                            return {
                                "valid": False,
                                "error": f"Table '{bad_table}' doesn't exist. Did you mean 'lab_results'?",
                            }
                        ...
                        if similar_tables:
                            ...
                            return {
                                "valid": False,
                                "error": f"Table '{bad_table}' doesn't exist in the database. {suggestion}",
                            }

                        return {
                            "valid": False,
                            "error": f"Table '{bad_table}' doesn't exist in the database. Check the schema for available tables.",
                        }
                    ...
                    if col_match:
                        ...
                        if (
                            ...
                            return {
                                "valid": False,
                                "error": "The vitals table doesn't have a 'test_name' column. It has direct columns like sbp, dbp, weight, etc.",
                            }
                        ...
                        if (
                            ...
                            if "patients.patient_id" in sql_query.lower():
                                return {
                                    "valid": False,
                                    "error": "The patients table uses 'id' not 'patient_id' as its primary key column.",
                                }

                        # Check for date vs test_date confusion
                        if (
                            bad_column.lower() == "test_date"
                            and "vitals" in sql_query.lower()
                        ):
                            return {
                                "valid": False,
                                "error": "The vitals table uses 'date' not 'test_date' for the date column.",
                            }
                        ...
                        if similar_columns:
                            ...
                            return {
                                "valid": False,
                                "error": f"Column '{bad_column}' doesn't exist in the referenced tables. {suggestion}",
                            }

                        return {
                            "valid": False,
                            "error": f"Column '{bad_column}' doesn't exist in the referenced tables.",
                        }
    ...
    def _validate_schema_references(self, references):
        ...
        for table in tables:
            if table not in self.table_details:
                ...
                if table + "s" in self.table_details:
                    ...
                    return {
                        "valid": False,
                        "error": f"Table '{table}' doesn't exist. Did you mean '{table}s'? Our database uses plural table names.",
                    }
                ...
                if table in ["lab", "labs"]:
                    return {
                        "valid": False,
                        "error": f"Table '{table}' doesn't exist. Did you mean 'lab_results'?",
                    }
                ...
                if similar_tables:
                    ...
                    return {
                        "valid": False,
                        "error": f"Table '{table}' doesn't exist in the database. {suggestion}",
                    }

                return {
                    "valid": False,
                    "error": f"Table '{table}' doesn't exist in the database schema.",
                }
        ...
        for column in columns:
            ...
            for table in tables:
                if table in self.table_details:
                    table_columns = self.table_details[table].get("columns", [])
                    if column in table_columns or column.lower() in [
            ...
            if not column_found:
                ...
                if column == "patient_id" and "patients" in tables:
                    invalid_columns.append(f"'{column}' (use 'id' in patients table)")
                    ...
                    invalid_columns.append(f"'{column}' (use 'date' in vitals table)")
                else:
        ...
        if invalid_columns:
            return {
                "valid": False,
                "error": f"Invalid column references: {', '.join(invalid_columns)}",
            }
    ...
    def _get_database_path(self):
        ...
        try:
            ...
            logger.warning("Could not get database path from db_query module: %s", exc)
            return None
    ...
    def _attempt_sql_fix(self, sql_query, error_msg):
        ...
        # Handle ambiguous column references
            ...
            if col_match:
                ...
                logger.info(f"Attempting to fix ambiguous column: {ambiguous_col}")
                ...
                for table, table_info in self.table_details.items():
                    if ambiguous_col.lower() in [
                        col.lower() for col in table_info["columns"]
                    ]:
                ...
                if tables_with_col:
                    ...
                    if len(tables_with_col) == 1:
                        ...
                        pattern = (
                            r"(?<!\w\.)(\b" + re.escape(ambiguous_col) + r"\b)(?!\.\w)"
                        )
